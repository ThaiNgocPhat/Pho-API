...
export class ToppingService {
    constructor(
        @InjectModel(DbCollections.TOPPING) private readonly toppingModel: Model<Topping>
    ){}
    ...
    async updateTopping(id: string, input: { name: string }): Promise<Topping> {
        const updateTopping = await this.toppingModel.findByIdAndUpdate(id, input, { new: true }).select('_id name');;
    ...
    async deleteTopping(id: string): Promise<void> {
        const deleteTopping = await this.toppingModel.findByIdAndDelete(id).select('_id');
